...
def get_rules() -> List[Dict]:
    ...
    rules = [
        ...
        },
        {
            ...
            'pattern': re.compile(r'NPM_TOKEN\s*[:=]\s*["\']?([A-Za-z0-9\-_]{20,})["\']?'),
            'score': 85
        },
        {
            'id': 'openvsx_token',
            'description': 'Open VSX token reference',
            'pattern': re.compile(r'OPENVSX_TOKEN\s*[:=]\s*["\']?([A-Za-z0-9\-_]{20,})["\']?'),
            'score': 85
        },
        {
            'id': 'openvsx_pat',
            'description': 'Open VSX PAT reference',
            'pattern': re.compile(r'OPENVSX_PAT\s*[:=]\s*["\']?([A-Za-z0-9\-_]{20,})["\']?'),
            'score': 85
        },
        {
            'id': 'github_token_generic',
            'description': 'GITHUB_TOKEN with suspicious value',
            'pattern': re.compile(r'GITHUB_TOKEN\s*[:=]\s*["\']?([A-Za-z0-9\-_]{20,})["\']?'),
            'score': 70
        },
        {
            'id': 'azure_client_secret',
            'description': 'Azure client secret assignment',
            'pattern': re.compile(r'AZURE_CLIENT_SECRET\s*[:=]\s*["\']?([A-Za-z0-9\-_~\.]{20,})["\']?'),
            'score': 80
        },
        {
            'id': 'aws_secret_key',
        ...
        },
        {
            ...
            'pattern': re.compile(r'(?i)(api[_-]?key|apikey|access[_-]?key)\s*[:=]\s*["\']?([A-Za-z0-9\-_]{24,})["\']?'),
            'score': 60
        },
        {
            'id': 'generic_secret',
            'description': 'Generic secret pattern',
            'pattern': re.compile(r'(?i)(secret|password|passwd)\s*[:=]\s*["\']?([A-Za-z0-9\-_!@#$%^&*]{16,})["\']?'),
            'score': 50
        },
        {
